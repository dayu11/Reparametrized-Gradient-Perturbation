...
def lrk_conv3x3(in_planes, out_planes, stride=1, rank=1, method='rs', group=1, kernel_size=3, padding=1):
    return LrkConv2d(in_planes, out_planes, stride=stride, rank=rank, method=method, group=group, kernel_size=kernel_size, padding=padding)
...
def weight_decomposition(W, R=None, rank=1, method='rs'):
    ...
    if(method == 'pi'):
        ...
        dim = min(W.shape)
        selected_ranks = np.random.choice(dim, rank, replace=False)

        L = W[:, selected_ranks]
        R = W[selected_ranks, :].T
        orthogonalize(L)
        orthogonalize(R)
    R = R.T
...
class LrkConv2d(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride, rank=1, method='rs', group=1, kernel_size=3, padding=1):
    ...
    def _decomposite_weight(self):
        ...
        wL, wR, self.approx_error = weight_decomposition(target, rank=self.rank, method=self.method)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, rank, method, stride=1, downsample=None):
        super(BasicBlock, self).__init__()
        self.rank = rank
        self.conv1 = lrk_conv3x3(inplanes, planes, stride, rank=self.rank, method=method)

        self.relu = nn.ReLU(inplace=True)
        self.conv2 = lrk_conv3x3(planes, planes, rank=self.rank, method=method)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, hyper_params_dict, num_classes=10):
        ...
        self.method = hyper_params_dict['method']

        self.num_layers = sum(layers)
        self.inplanes = 16 * k
        self.conv1 = lrk_conv3x3(3, 16 * k, rank=self.rank, method=self.method)

        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(block, 16 * k, layers[0], self.rank, self.method)
        self.layer2 = self._make_layer(block, 32 * k, layers[1], self.rank, self.method, stride=2)
        self.layer3 = self._make_layer(block, 64 * k, layers[2], self.rank, self.method, stride=2)
    ...
    def _make_layer(self, block, planes, blocks, rank, method, stride=1):
        downsample = None
        if(use_bn):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, rank, method, stride, downsample))
        self.inplanes = planes
        for _ in range(1, blocks):
            layers.append(block(planes, planes, rank, method))
